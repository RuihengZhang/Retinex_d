...
def variabletrain(device, decompose_mode, denoising_mode): 
    model = RetinexFormer(stage=1,n_feat=40,num_blocks=[1,2,2])     # 为了尽可能快的训练，使用单卡训练
...
def retinexformer(img, decompose_mode=None):
    model = RetinexFormer(stage=1,n_feat=40,num_blocks=[1,2,2]) 
    if decompose_mode == None:
        model.load_state_dict(torch.load(f'/mnt/jixie8t/zd_new/Code/RetinexD/Checkpoints/lightening/retinexformer/retinexformer_0to2.pth'))
    else:
        ckt_path = f'/mnt/jixie8t/zd_new/Code/RetinexD/Checkpoints/lightening/retinexformer/retinexformer_0to2_with_retinexd.pth'
    ...
    return derepeated_tensor.cpu()[0].numpy()
