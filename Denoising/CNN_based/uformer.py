...
def variabletrain(device, decompose_mode, denoising_mode): 
    model = Uformer(img_size=256, embed_dim=16,depths=[2, 2, 2, 2, 2, 2, 2, 2, 2],win_size=8, mlp_ratio=4., token_projection='linear', token_mlp='leff', modulator=True, shift_flag=False)      # 为了尽可能快的训练，使用单卡训练
...
def uformer(img, decompose_mode=None):
    model = Uformer(img_size=256, embed_dim=16,depths=[2, 2, 2, 2, 2, 2, 2, 2, 2],win_size=8, mlp_ratio=4., token_projection='linear', token_mlp='leff', modulator=True, shift_flag=False)
    if decompose_mode == None:
        model.load_state_dict(torch.load(f'/mnt/jixie8t/zd_new/Code/RetinexD/Checkpoints/denoising/uformer/uformer_0to1.pth'))
    else:
        ckt_path = f'/mnt/jixie8t/zd_new/Code/RetinexD/Checkpoints/denoising/uformer/uformer_0to1_with_retinexd.pth'
